...
class SVGTensorDataset(torch.utils.data.Dataset):
    ...
    def preprocess(svg, augment=True, numericalize=True, mean=False):
        if augment:
            svg = SVGTensorDataset._augment(svg, mean=mean)
        if numericalize:
...
class SVGFinetuneDataset(torch.utils.data.Dataset):
    ...
    def __len__(self):
        return math.ceil(len(self.svg_list) / self.frac) * self.nb_augmentations

    def __getitem__(self, idx):
        i = idx % math.ceil(len(self.svg_list) / self.frac)
        if i < len(self.svg_list):
            return self.original_dataset.get(svg=self.svg_list[i].copy())
        else:
            return self.original_dataset.random_icon()
